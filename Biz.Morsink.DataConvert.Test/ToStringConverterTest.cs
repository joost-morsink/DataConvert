...
namespace Biz.Morsink.DataConvert.Test
{
    ...
    public class ToStringConverterTest
    {
        ...
        {
            var conv = new DataConverter(new[] { new ToStringConverter(false) });
        ...
        public void ToString_SucceedOnNull() {
            var conv = new DataConverter(new[] { new ToStringConverter(true) });
        ...
        {
            var conv = new DataConverter(new[] { new ToStringConverter(false, CultureInfo.GetCultureInfo("nl-nl")) });
            Assert.AreEqual("1234,56", conv.Convert(1234.56m).To<string>());
        }
        [TestMethod]
